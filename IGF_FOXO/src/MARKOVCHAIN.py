...
import time
import pandas as pd
from PredictionFunction_v2 import  get_prec_preds
...
def RunMCMC(outpath, Lagrange_multi,  Cons_Bound_dict, abund_low_lim, abund_upp_lim,   params_upperbound , params_lowerbound , L, times_arr,
    ...
    Inputs: outpath: Path in which the files will be written
            ...
            cell_sense_thresh = .1 , is used to discard cells that are not sensitive  
             """
    ...
    for i in range(N_chain):
        ...
        if si%save_step ==0 :
            ...
            if ss > ignore_steps: 
                ...
                if timethis==True:
                    t = time.time()
                outarr = np.concatenate((pred_curr,mu_curr, v_curr, k ), axis = 0)
                ...
                if si%int(N_chain/2)==0:
                    ...
                    line = [iteration,'acc' , A_Ratio, 'rej_flag_par', RJ_Ratio_Par, 'rej_flag_abund', RJ_Ratio_Ab\
                        , 'rej_flag_sensitivity', RJ_Ratio_Sens , 'deltaE', deltaE, 'step time' ,np.round(time.time()-ts)/(i+1)]
    ...
    write_output(info_filename, line )
    tf = time.time()
    print(f'MCMC for each step in iteration {iteration} took {np.round(tf-ts)/N_chain} seconds')

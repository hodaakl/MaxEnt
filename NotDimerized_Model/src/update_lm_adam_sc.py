...
import os
import pandas as pd
from update_lm_fns import calculate_constraints, update_lambda, openfile, update_lambda_adam
...
# must index Preds bec that file containts [percentile predictions , means predictions , variance predictions , paramteters] concatenated 
# but we are interested only in the percentile predictions for the error 
...
Error  = Error.tolist()
##### IF NOT ADAPTIVE METHOD UNCOMMENT THE FOLLOWING AND COMMENT THE PREVIOUS BLOCK
# alpha = .002
# # Update the lagrange multipliers
# Lambda = update_lambda(Error = Error, old_lambda= Old_Lambda, alpha_cons = alpha)#, alpha_power = alpha_power) 
# Lambda= Lambda.tolist()
...
##### MUST DO FOR NEXT ITERATION TO RUN

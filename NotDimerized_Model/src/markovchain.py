...
import time
import pandas as pd
from prediction_fns import get_prec_preds
...
def RunMCMC(outpath, Lagrange_multi,  Cons_Bound_dict, abund_low_lim, abund_upp_lim,   params_upperbound , params_lowerbound , L,\
    ...
    Inputs: outpath: Path in which the files will be written
            ...
            noise_factor: artificially increasing the coefficient of variation of single cell dist by a noise_factor \
                default:1
            ...
            predfactor: to scale the solutions if needed. default:1 
    Outputs:          
            cellpreds_{iteration}.csv -- the simulated population , explained columns in ../dictionary_out.txt
            info.csv  -- information about the run, acceptance and rejection ratios, time taken by simulation etc.
             """
    ...
    for i in range(N_chain):
        ...
        if si%save_step ==0 :
            ...
            if ss > ignore_steps: 
                ...
                # this prevents interference with other currently running MCMCs that mess up the row order, we want 1 row = 1 cell consistently.

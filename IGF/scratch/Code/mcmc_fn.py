...
def RunMCMC(outpath, Lagrange_multi,  Cons_Bound_dict, abund_low_lim, abund_upp_lim,   params_upperbound , params_lowerbound , L, times_arr,
     N_chain= 10, save_step = 2, ignore_steps = 2, iteration = 1 , param_range = 0.1, num_par_change = 5, \
         timethis=False, predfactor=1, cell_sense_thresh = .2, integ_method = 'RK45'):#, nBins = 10):
    ...
    k = initial_K
    # Check bounds of FoxO 
    Flag_sens  = Get_sensitivity_flag(k,thresh =cell_sense_thresh , timethis = timethis, meth = integ_method)
    Flag_abund = 0
    if Flag_sens == 1:
    ...
    for i in range(N_chain):
        ...
        if Flag_params==1: 
            ### 
            t0=time.time()
            Flag_sens  = Get_sensitivity_flag(new_k,thresh =cell_sense_thresh , timethis = timethis , meth = integ_method )
            # print(f'Assessed the sensitivity in {time.time()-t0} seconds')

...
from PredictionFunctions import  cell_pred_fn, get_prec_preds
...
## 
def RunMCMC(outpath, Lagrange_multi,Cons_Bound_dict, params_upperbound = highbound, params_lowerbound = lowbound,  N_chain= 10, save_step = 2, ignore_steps = 2, iteration = 1 , param_range = 0.1, num_par_change = 5, timethis=True):#, nBins = 10):
...
#         K_curr =  np.random.uniform(low = par_low/2 , high = par_high/2) # current parameters
    ...
    if timethis==True:
        ...
        t = time.time()
    # ------------------------ 
    Lagrange_multi = Lagrange_multi
    # nCons = len(Lagrange_multi)
    k = initial_K
    # means_arr, var_arr = cell_pred_fn(k, times_arr, L ) 
    pred_curr = get_prec_preds(Cons_Bound_dict, k, times_arr, L)
    if timethis == True:
        t1 = time.time()
...
#         print(i)
        ...
        if Flag_params ==1 : 
            ...
            pred_new  = get_prec_preds(Cons_Bound_dict, new_k, times_arr, L)
